...
2. **Development**: Implement the planned features and fixes in `feature/*` branches, which are then merged into the `master` branch via pull requests.
...
5. **Release Preparation**: Create a `release/*` branch from the `master` branch, finalize release-specific changes, and update version numbers.
6. **Release**: Merge the `release/*` branch into the `master` branch, tag the release.
7. **Post-Release**: Verify the release artifacts and notify stakeholders about the release.
...
5. If documentation was updated, generate the documentation for the new version using the release script:
   ...
   yarn release:doc
   ```
6. Merge the changelog PR automatically generated by the changelog pipeline into the `release/*` branch.

**2. Release Steps**

1. Create a PR for the `release/*` branch to the `master` branch.
...
1. Verify the tag was created on the `master` branch.
2. Verify the Docker images were published to GHCR.
3. Notify relevant stakeholders about the release.
...
3. Merge the changelog PR automatically generated by the changelog pipeline into the `hotfix/*` branch.
4. Create a PR for the `hotfix/*` branch to the `master` branch.
5. Get approval from the team.
   ...
   - The PR must pass all automated tests and have no conflicts.
6. Merge the PR to the `master` branch.
...
1. All feature development occurs in `feature/*` branches branched from the `master` branch.
2. Features are merged back into the `master` branch upon completion and successful review.

### Release Branches

1. Create a `release/*` branch from the `master` branch to prepare for a new version.

- In some special cases, the release wants to cherry-pick some features. Create the branch from the last release tag and cherry-pick the required commits.

2. Use this branch to finalize release-specific changes such as version bumping and documentation updates.
3. Merge the `release/*` branch into the `master` branch to release.
4. Tag is automatically created on `master` with the release version (e.g., `1.1.0`) by the release pipeline.

### Hotfix Branches

1. In the event of a critical issue, create a `hotfix/*` branch from the `master` branch.
2. Make and commit the necessary changes.
3. Merge the `hotfix/*` branch into the `master` branch and tag is automatically created (e.g., `1.1.1`).
...
flowchart TD
    ...
    I -->|Error| F
    I --> J[Create Release Changelog]
    J --> K[End]
```

### Release Pipeline

The release pipeline will automatically tag the release version when the `release/*` branch or `hotfix/*` is merged into the `master` branch. The release pipeline will run the unit test and build the application to ensure the code is stable before releasing.
The release is initiated by the `release` job in the pipeline.

```mermaid
flowchart TD
    A[Start] --> B{Trigger Event}
    B -->|Push to master| C[test_and_build Job]
    B -->|Manual Trigger| C
