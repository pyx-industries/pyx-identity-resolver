...
```
https://your-resolver.example.com/api/1.0.0/acme/product/12345/batch/A1B2C3?linkType=acme:sustainabilityInfo
```
...
They follow a namespaced convention like `acme:sustainabilityInfo` or `acme:productDatasheet`.

Link types come from controlled vocabularies.
You can use standard vocabularies — such as those defined by GS1 or other industry bodies —
or define your own custom link types to suit your domain.
The resolver does not impose a fixed set of link types;
you as the registry operator decide what link types are meaningful
for your identifier scheme.
...
```
https://your-resolver.example.com/api/1.0.0/gs1/01/09359502000010/10/ABC123?linkType=gs1:certificationInfo
```
