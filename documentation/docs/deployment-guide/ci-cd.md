...
3. If the tag is new:
   - Creates an annotated Git tag (e.g. `1.1.3`).
...
| `apiVersion` | The API contract version. Only updated when the API surface changes. Used in the URL path (`/api/1.0.0`). |
...
|-------------|---------|-------------------|-----------------|
| `X.Y.Z` | `1.1.3` | On semver tag push | **Production** --- pin to a specific version. |
| `X.Y.Z-serverless` | `1.1.3-serverless` | On semver tag push | **Production Lambda** --- pin to a specific version. |
...
docker pull ghcr.io/pyx-industries/pyx-identity-resolver:1.1.3
...
```bash
# Specific version (recommended)
docker pull ghcr.io/pyx-industries/pyx-identity-resolver:1.1.3-serverless
